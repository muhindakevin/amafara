...
function Layout({ children, userRole = 'Member' }) {
  ...
  const allItemsForRole = menuItems[userRole] || menuItems.Member

  // Filter items based on granular permissions
  const items = allItemsForRole.filter(item => {
      ...
      <div className="flex relative pt-[60px] sm:pt-[65px] md:pt-[73px]">
        ...
        <aside
          className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
            } fixed lg:fixed top-[60px] sm:top-[65px] md:top-[73px] left-0 z-40 h-[calc(100vh-60px)] sm:h-[calc(100vh-65px)] md:h-[calc(100vh-73px)] w-64 sm:w-72 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-sm transform transition-transform duration-300 ease-in-out`}
        ...
        >
          <nav className="p-3 sm:p-4 space-y-1 sm:space-y-2">
            {items.map((item, index) => {
              ...
              return (
                <Link
                  key={index}
                  to={item.path}
                  className="flex items-center gap-2 sm:gap-3 px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl hover:bg-primary-50 dark:hover:bg-gray-700 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-200 group text-sm sm:text-base"
                >
                  <Icon size={18} className="text-gray-600 dark:text-gray-300 group-hover:text-primary-600 dark:group-hover:text-primary-400 flex-shrink-0 sm:w-5 sm:h-5" />
                  <span className="font-medium text-gray-700 dark:text-gray-300 group-hover:text-primary-600 dark:group-hover:text-primary-400 truncate">{translatedLabel}</span>
                </Link>
              )
            })}
            <button
            ...
            >
              <LogOut size={18} className="text-gray-600 dark:text-gray-300 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors duration-200 flex-shrink-0 sm:w-5 sm:h-5" />
              <span className="font-medium text-gray-700 dark:text-gray-300 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors duration-200">{tCommon('logout', { defaultValue: 'Logout' })}</span>
            </button>
          </nav>
        </aside>
        ...
        <main
          className={`flex-1 transition-all duration-300 ${sidebarOpen ? 'lg:ml-64 xl:ml-72' : 'lg:ml-0'} p-3 sm:p-4 md:p-6 bg-transparent`}
          style={{
